...
Place spaces around all infix operators (`=`, `+`, `-`, `<-`, etc.), and when naming arguments in function calls. Always put a space after a comma, and never before (just like in regular English).
...
The arguments to a function typically fall into two broad sets: one set supplies the __data__ to compute on, and the other supplies arguments that control the __details__ of the computation. When you call a function, you typically omit the names of the data arguments, because they are used so commonly. If you override the default value of a detail argument, you should use the full name:
