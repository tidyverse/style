...
top of your function documentation. Titles should use sentence case but not end 
with a full stop.
```{r}
...
For both the title and the description, there is no need to use the explicit 
`@title` or `@description` tag, except if the description is multiple 
paragraphs or includes more complex formatting like a bulleted list. 
```{r}
...
If some functions share parameters, you can use `@inheritParams` to keep the 
documentation in one place and avoid duplication descriptions in multiple places.
```{r}
...
case sensitive. Use a double point before you start to enumerate.
```{r}
...
`slice`, `summarize` form the family single table verbs.
```{r}
...
If you want insert a line break within a section, also leave a line break in the 
`.Rd` file.
```{r}
