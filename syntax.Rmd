...
Most infix operators (`==`, `+`, `-`, `<-`, etc.) should be surrounded by
spaces. The exception are those with relatively [high precedence](http://stat.ethz.ch/R-manual/R-patched/library/base/html/Syntax.html):
`^`, `:`, `::`, and `:::`. To highlight operator precedence, use parentheses rather than irregular spacing.
