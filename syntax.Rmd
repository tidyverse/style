...
Write the complete argument name --never abbreviate.  This is important when developing stable code (in a package, for example).  Partial matching is suitable only for interactive use. 

Although the R execution tries to save you time by matching a partially-completed name, do not rely on this.  Compared to complete matching, partial matching is less consistent and less readable.

A second problem is the function signature may later gain a new argument which conflicts with your partial match.
