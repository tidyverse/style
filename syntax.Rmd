...
You may also place multiple unnamed arguments on the same line if they are closely
related to each other, e.g., strings in calls to `paste()` or arguments to `sprintf()`. When
building strings, match one line of code to one line of output where possible.
