...
If the function name and definition can't fit on a single line, place each argument on its own double indented line. The trailing `)` and leading `{` should go on the same line as the last argument.
...
# No longer recommended because it doesn't work very well when the function
# name is very long, which is not uncommon with S3 methods.
...
If a function argument can't fit on a single line, this is a sign you should extract out the long default argument into its own helper function.
